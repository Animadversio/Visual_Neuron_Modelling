...
while idx_csr < len(stimpaths):
    ...
    input_tsr = np.concatenate(tuple(ppimgs), axis=0)
    output = sess.run(net.conv4, feed_dict={net.x: input_tsr})
    out_feats_all = np.concatenate((out_feats_all_M, output), axis=0) if out_feats_all_M.shape else output
    idx_csr = idx_ub
    BS_num += 1
    print("Finished %d batch, take %.1f s" % (BS_num, time() - t0))
t1 = time()
# Temporially safe files
np.savez("Mfeat_tsr.npz", feat_tsr=out_feats_all_M, ephysFN=MData.ephysFN, stimuli_path=MData.stimuli)
print("%.1f s" % (t1 - t0))
...
pred_score = RdgCV.transform(out_feats_all_M.reshape((out_feats_all.shape[0], -1)))
